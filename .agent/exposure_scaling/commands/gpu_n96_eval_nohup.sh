...
LOG="$LOG_DIR/gpu_n96_eval_${TS}.log"
PREFLIGHT_LOG="$LOG_DIR/gpu_cache_preflight_n96_eval_${TS}.log"
PID_FILE="$LOG.pid"
...
nohup bash .agent/exposure_scaling/commands/gpu_n96_eval.sh > "$LOG" 2>&1 &
PID=$!
echo "$PID" > "$PID_FILE"

echo "started gpu_n96_eval.sh"
echo "pid: $PID"
echo "log: $LOG"

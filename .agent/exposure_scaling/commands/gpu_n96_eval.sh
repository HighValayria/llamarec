...
# N-K0 96k cloud evaluation on fixed k5_popmatch_seed42 candidates.
...
N96=outputs/n/movielens-1m/exposure_n_s12000
...
require_dir "$N96/adapter"

python -m src.inference.evaluate_n_adapter \
  --config configs/n_local_model.yaml \
  --dataset movielens-1m \
  --adapter-dir "$N96/adapter" \
  --mode real \
  --splits validation test \
  --batch-size 1 \
  --valid-candidates "$VALID_POPMATCH" \
  --test-candidates "$TEST_POPMATCH" \
  --output-dir "$N96/popmatch_eval"

cat <<'MSG'

N96 evaluation finished. Compare N24/N48/N96 validation first; test remains report-only.
MSG

...
# N-K0 96k cloud training for LLM Exposure Scaling & Convergence Validation.
# Validation-first decision on 2026-08-29: N24 -> N48 improved, so continue N only.
...
N48=outputs/n/movielens-1m/exposure_n_s6000/checkpoints/checkpoint-6000
require_resume_state "$N48"

# N-K0 96k: 12000 steps * 8 samples/step = 96000 N exposure.
python -m src.train.train_n \
  --config configs/n_local_model.yaml \
  --dataset movielens-1m \
  --run-name exposure_n_s12000 \
...
N96 training finished. Next required step:
Run fixed PopMatch validation/test evaluation with gpu_n96_eval_nohup.sh.
MSG
